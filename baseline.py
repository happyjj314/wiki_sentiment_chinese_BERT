from dataset.applysentenceDataset import preprocessing
...
class Sentiment_Analysis:
    ...
    def __call__(self, text_list, batch_size=1, threshold=.52):
        ...
        for i in range(n_batches):
            ...
            predictions = self.bert_model.forward(text=texts_tokens_,
    ...
    def sentiment_print_func(self, text, pred, threshold):
        print(text)
        if pred >= threshold:
            print("满意, 输出值{:.2f}".format(pred))
        else:
            print("不满意, 输出值{:.2f}".format(pred))
        print("----------")
