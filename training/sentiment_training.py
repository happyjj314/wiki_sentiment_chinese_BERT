...
class Sentiment_trainer:
    def __init__(self,max_len,batch_size,lr,with_cuda= True):
        ...
        train_dataset = SentimentDataset(corpus_path=self.config["train_corpus_path"],
        ...
        test_dataset = SentimentDataset(corpus_path=self.config["test_corpus_path"],
                                  word2idx=self.word2idx,
                                  max_seq_len=self.max_len,
    ...
    def iteration(self,epoch,data_loader,train=True,df_name="df_log_sentiment.pickle"):
        ...
        for i, data in data_iter:
            ...
            # 正向传播, 得到预测结果和loss
            predictions, loss = self.bert_model.forward(text=data["text_input"],
        ...
        threshold_ = find_best_threshold(all_predictions, all_labels) #自己定义一个找阈值的
...
if __name__ == '__main__':
    def init_trainer(dynamic_lr, batch_size=128):
        trainer = Sentiment_trainer(max_len=300,
    ...
    for epoch in range(train_epoches):
        if not continue_train:
            ...
            trainer.load_model(trainer.bert_model, dir_path="../output_wiki_bert", load_bert=True)
        elif continue_train:
        ...
        if threshold >= patient:
            print("epoch {} has the lowest loss".format(np.argmax(np.array(all_auc))))
            print("early stop!")
            break
